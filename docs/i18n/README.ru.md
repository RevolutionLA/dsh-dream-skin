...
## 🎨 Способ №1: 8 пресетов скинов (серия Mirage)
...
## 🏆 Почему стоит попробовать (сравнение с альтернативами)

> Взгляните под другим углом: аналогичные плагины либо переносят готовую палитру (красиво, но настройка сводится
> к одному переключателю), либо фиксируют одну эстетику, либо занимаются только переносом внешних обоев.
> Мы же сделали смену скинов **полноценной настраиваемой системой материалов и цвета** — цель не «пестрее»,
> а «точнее, сдержаннее, долговечнее взгляду», как отполированное стекло. **Эстетика + настраиваемость — наш ров.**

| Возможность | Этот плагин | [dsh-catppuccin-theme](https://github.com/) (перенос палитры) | [dsh-theme-mineradio](https://github.com/) (единая эстетика) | [dsh-wallpaper-engine](https://github.com/) (мост обоев) |
|------|:---:|:---:|:---:|:---:|
| **8 оригинальных дизайнов** (не перенос готовой палитры: свои токены + рассеянное свечение) | ✅ | ❌ (4 официальные палитры Catppuccin) | ❌ (1 эстетика шампань-золота) | ❌ |
| **Матовое / жидкое стекло — два материала**, переключение в один клик | ✅ | частично (фиксированное стекло) | ❌ | ❌ |
| **Отдельные ползунки прозрачности для поля ввода / всплывающих окон** | ✅ | ❌ | ❌ | ❌ |
| **Готовый вид сразу после установки** (перезапустил — и всё уже настроено) | ✅ | ❌ | ✅ (сам по себе готовый продукт) | ❌ |
| Свои обои + прозрачность/размытие | ✅ | ❌ | ❌ | ✅ (ключевая возможность) |
| **Обои 2.0** (URL / пресеты градиентов / предложение для каждого скина / авто-затемнение / ежедневно с Bing + обновление по расписанию) | ✅ | ❌ | ❌ | частично (зависит от обоев WE) |
| **Индивидуальный Accent для пользователя** (слой поверх, сам скин не затрагивается) | ✅ | ❌ | ❌ | ❌ |
| **Импорт/экспорт тематических пакетов + ссылки для обмена** (JSON, распространение без кода) | ✅ | ❌ | ❌ | ❌ |
| Локальная библиотека пакетов + избранное + «удиви меня» | ✅ | ❌ | ❌ | ❌ |
| **Совместимость с двумя поколениями хоста + определение возможностей в рантайме** (при смене поколения хоста автоматически деградирует без ошибок) | ✅ | неизвестно | неизвестно | ❌ (сначала нужно обновить ядро) |
| Проверка + откат (без разрушающих изменений) | ✅ | частично | — | частично |

> **Коротко**: хотите фирменные цвета Catppuccin, атмосферу mineradio? Система тематических пакетов этого плагина
> способна это собрать или наложить поверх — но не наоборот.
...
| 🧊 **Материалы стекла (Матовое / Жидкое)** | Один клик меняет характер стекла; ползунки прозрачности: **вправо — прозрачнее**; ОДИН ползунок размытия управляет фоном и всеми стеклянными поверхностями |
| 🖼️ **Готовый вид сразу после установки** | Первый запуск уже полностью оформлен: тема Nebula + встроенные обои + настроенные значения — перезапустите и пользуйтесь |
| 🌤️ **Картинка дня Bing (предустановлена)** | В расширенных обоях заранее подставлен API картинки дня Bing — нажмите «Применить» и готово; также поддерживается любая ссылка на изображение с автообновлением |
...
- [x] Первая версия: 8 тем + свои обои (прозрачность / размытие) + локальное сохранение
...
- [x] Обои 2.0 (URL / градиент / предложение для каждого скина / авто-затемнение / ежедневно с Bing + обновление по расписанию)
- [x] Локальная библиотека пакетов + применение в один клик / избранное / «удиви меня»
- [x] Локализация и документация на многих языках (zh / en / ja / ko / es / fr / de / ru)
- [x] Система материалов стекла: матовое / жидкое стекло, два материала + отдельные ползунки прозрачности (v9.13.0)
- [x] Готовый вид сразу после установки: перезапустил — и уже полностью оформленный вид (v9.13.0)
- [x] Укрепление совместимости с хостом: определение рантайма для двух поколений хоста + DOM-маркеры форм на случай дрейфа имён классов (issue #50, v9.13.x)
- [ ] Полное устранение зависимости от хэшированных имён классов хоста: оставшиеся декоративные правила (сайдбар / панель файлов) тоже перевести на DOM-маркеры форм
- [ ] Онлайн-студия палитр / превью тем (чистый фронтенд, проверка в браузере + проверка контраста)
- [ ] Галерея тем сообщества (отправка пакетов в репозиторий / онлайн-галерею; добро пожаловать с производными палитрами вроде Catppuccin через тематические пакеты)
