...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
			...
			const check = () => {
				try {
					...
					if (isDesktopShell()) {
						...
						if (!hit) desktopMissed = DESKTOP_SIDEBAR_SELECTOR + " (desktop shell sidebar surface)";
					}
					const missed = hostMissed.concat(desktopMissed ? [desktopMissed] : []);
					if (missed.length > 0) {
						console.warn(
					...
					publishStatus({
						anchors: {
							probed: MATERIAL_SELECTOR_PROBES.length + (desktopMissed !== null ? 1 : 0),
							drifted: missed.slice()
						},
						checkedAt: Date.now()
					});
				} catch {}
		 ...
		 * no longer matches. The write is a normal user-state write, so it also
		 * overwrites the legacy copy in `$DSH_HOME/dream-skin.json`.
		 */
		...
		function migrateLegacyFactoryWallpaper() {
			try {
				...
				writeStorage(WALLPAPER_KEY, FACTORY_DEFAULTS[WALLPAPER_KEY]);
			} catch {}
		}
