...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function loadFromHost() {
			try {
				// Blue-team T2/A4: the probe must not hang forever — a stalled host
				// (accepts the connection, never answers) would keep the push gate
				// shut for the whole session and silently defer every user write
				// to the next boot. 4s cap, then treat as unreachable.
				const res = await Promise.race([
					...
					new Promise((_, reject) => setTimeout(() => reject(new Error("host probe timeout")), 4000))
				]);
		...
		function ensureMaterialStyle() {
			...
			const parts = [
				...
				'html[data-dsh-material="liquid"] .uV2eYG_card::before {',
				"  -webkit-backdrop-filter: blur(calc(var(" + GLASS_BLUR_VAR + ", " + DEFAULT_GLASS_BLUR + "px) + var(" + LIQUID_THICKNESS_VAR + ", 10px))) saturate(1.8) contrast(1.04) brightness(1.02);",
				"  backdrop-filter: blur(calc(var(" + GLASS_BLUR_VAR + ", " + DEFAULT_GLASS_BLUR + "px) + var(" + LIQUID_THICKNESS_VAR + ", 10px))) saturate(1.8) contrast(1.04) brightness(1.02);",
				"}",
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)(Slider, {
						...
						step: 1,
						format: (v) => `${v}%`,
						onChange: (v) => setSidebarOpacity(100 - v)
					}),
					(0, react_jsx_runtime.jsx)("div", {
						style: styles.actionRow,
						children: [
		...
		function applyFactoryDefaults() {
			...
			if (readStorage(FACTORY_APPLIED_KEY) != null || hasAnyStoredValue) {
				if (readStorage(FACTORY_APPLIED_KEY) == null) writeStorage(FACTORY_APPLIED_KEY, "1");
				return;
			}
