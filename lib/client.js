	...
	factory: (require) => {
		...
		function rawActiveTheme(snapshot) {
			...
			const selectedId = typeof savedId === "string" && savedId !== DEFAULT_SKIN
				...
				? snapshot.active?.id
				: snapshot.preference;
			const selected = snapshot.themes?.find((theme) => theme.id === selectedId)
				|| snapshot.themes?.find((theme) => theme.id === snapshot.active?.id)
				|| snapshot.active;
			try {
				document.documentElement.dataset.dreamSkinTrace = JSON.stringify({
					build: "yukiryou.1",
					savedId,
				...
				// Diagnostic marker is best-effort in the fork test build.
			}
			return selected;
		}
		...
		function applyCombinedTokenOverrides(ctx) {
			...
			try {
				...
				try {
					document.documentElement.dataset.dreamSkinCombinedTrace = JSON.stringify({
						build: "yukiryou.2",
						base: overrides["--dsw-alias-bg-base"],
						sidebar: overrides["--dsw-specific-sidebar-fill"]
					});
				} catch {
					// Diagnostic marker is best-effort in the fork test build.
				}
		...
		function shadeTokens2(ctx, canvasAlpha, snapshot = null) {
			...
			const active = rawActiveTheme(current);
			try {
				document.documentElement.dataset.dreamSkinShadeTrace = JSON.stringify({
					build: "yukiryou.2",
					preference: current.preference,
					activeId: current.active?.id,
					resolvedId: active?.id,
					resolvedScheme: active?.colorScheme,
					resolvedBase: active?.tokens?.["--dsw-alias-bg-base"]
				});
			} catch {
				// Diagnostic marker is best-effort in the fork test build.
			}
