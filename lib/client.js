...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function warnOnMaterialSelectorDrift() {
			...
			const check = () => {
				try {
					...
					const hostMissed = MATERIAL_SELECTOR_PROBES.filter((sel) => {
						try { return document.querySelector(sel) === null; } catch { return true; }
					});
					let desktopMissed = null;
					if (isDesktopShell()) {
						let hit = false;
						try { hit = document.querySelector(DESKTOP_SIDEBAR_SELECTOR) !== null; } catch { hit = false; }
						// Keep the machine-readable list EXACT selectors only —
						// the human label goes to the console line (9.26.1, B-08:
						// decorated strings are not valid selectors).
						if (!hit) desktopMissed = DESKTOP_SIDEBAR_SELECTOR;
					}
					const missed = hostMissed.concat(desktopMissed ? [desktopMissed + " (desktop shell sidebar surface)"] : []);
					if (missed.length > 0) {
						console.warn(
					...
					// Machine-readable mirror of the same verdict: host/desktop
					// tooling reads `anchors` from window.__DSH_DREAM_SKIN_STATUS__
					// instead of parsing console output. A zero-length `drifted`
					// list is the positive signal that every refinement is live.
					publishStatus({
						anchors: {
							probed: MATERIAL_SELECTOR_PROBES.length + (isDesktopShell() ? 1 : 0),
							drifted: hostMissed.concat(desktopMissed ? [desktopMissed] : [])
						},
						checkedAt: Date.now()
					});
				} catch {}
		...
		function isSafeWallpaperGradient(value) {
			...
			if (/(url\s*\(|image-set\s*\(|element\s*\(|cross-fade\s*\(|-moz-element|expression\s*\(|@import|javascript:)/i.test(v)) return false;
			// The value must be a gradient stack (a bare color or any other image
			// kind is out of scope here). After the denylist, remaining syntax can
			// only paint — it can no longer reference an external resource.
			return /^(?:linear|radial|conic)-gradient\(/i.test(v);
		}
		...
		function wallpaperBackgroundCss() {
			...
			if (kind === "gradient") {
				...
				if (grad === null || !isSafeWallpaperGradient(grad)) return null;
				return grad;
			}
			if (kind === "url") {
				const url = readWallpaperUrl();
		...
		const FACTORY_DEFAULTS = {
			...
			[WALLPAPER_KEY]: "data:image/jpeg;base64,",
		...
		/**
		 * One-time replacement of the factory wallpaper shipped in v9.13.0–v9.23.0.
		 * The shipped default was a stock photo that does not fit the brand and
		 * carries no distribution license; this build ships an original abstract
		 * diffused-glow image. Users who still have the old factory image — in
		 * localStorage or in the host state file — get it replaced with the new
		 * one; anything else (a custom photo, a cleared wallpaper) is left alone.
		 * Identity is the exact stored-string length + decoded byte length +
		 * cyrb53 of the JPEG bytes, so a false positive against a user photo is
		 * not realistically possible, and the length pre-check avoids decoding
		 * anything on the common path. Idempotent: once replaced, the new value
		 * no longer matches. Write semantics follow the boot probe: BEFORE the
		 * host state settles the swap is PROVISIONAL (factory-sealed — a durable
		 * host value, including a user-cleared "" wallpaper, still outranks it,
		 * issue #51 semantics); AFTER settlement it is a normal user-state write,
		 * so the legacy copy living in `$DSH_HOME/dream-skin.json` gets replaced
		 * too and every origin converges on the new image.
		 */
		const LEGACY_FACTORY_WALLPAPER = {
			dataUrlLength: 115863, // "data:image/jpeg;base64," + 115840 base64 chars
			byteLength: 86879,
			hash: 1042845555783671
		};
		function migrateLegacyFactoryWallpaper() {
			try {
				const raw = readStorage(WALLPAPER_KEY);
				if (typeof raw !== "string" || raw.length !== LEGACY_FACTORY_WALLPAPER.dataUrlLength) return;
				if (raw.indexOf("data:image/jpeg;base64,") !== 0) return;
				const bin = atob(raw.slice(23));
				if (bin.length !== LEGACY_FACTORY_WALLPAPER.byteLength) return;
				...
				if (4294967296 * (2097151 & h2) + (h1 >>> 0) !== LEGACY_FACTORY_WALLPAPER.hash) return;
