...
> **面板可读性修复版**：深色皮肤下打开设置时，面板里会透出对话页面的文字。

### 根因（已代码实锤）
- 深色皮肤把 `--dsw-alias-bg-layer-2` 写成 `rgba(…, 0.85)`（5 套深色皮肤一致）。该 token 承载设置面板等**承载文字**的表面，15% 的穿透让下层对话文字与面板文字叠在一起；
- 与 issue #27 已修复的 `--dsw-alias-bg-layer-1` 属同一类缺陷，只是深了一层语义：`bg-layer-1` 已强制为不透明实底，`bg-layer-2` 未做同样处理；
- 与壁纸 wash / 侧栏透明度无关：这个 alpha 是**逐皮肤写死**的常量，不受任何滑杆控制，因此调滑杆无法缓解。

### 修复
- 5 套深色皮肤（`abyss` / `aurora` / `nebula` / `ember` / `midnight`）的 `--dsw-alias-bg-layer-2` 由 `0.85` 提升到 `0.96`：保留色调与「面板略浮于壁纸之上」的观感，同时让穿透低到不再读出文字；
- **不动 `mist`**：该皮肤的文档化设计就是「清透半透明白 + blur」的液态玻璃，改动它等于取消其设计意图；
- 新增防回归断言：内置皮肤的 `--dsw-alias-bg-layer-2` 必须不透明或 α ≥ 0.95（`mist` 按名豁免至 0.5，理由是上述设计意图）。断言已在 `0.85` 上验证会失败，确保同类回归不能悄悄回来。
